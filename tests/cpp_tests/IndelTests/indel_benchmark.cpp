...
int main() {
    ...
    while (counter++ < 100) {
        ...
        auto indel_duration = std::chrono::duration_cast<std::chrono::milliseconds>(end - start);
        std::cout << "indel time: " << indel_duration.count() << "ms" << "\n";

        start = std::chrono::high_resolution_clock::now();
        MSA msa = MSA(blockmap, tree_.getRoot(), saveList);
        end = std::chrono::high_resolution_clock::now();
        auto msa_duration = std::chrono::duration_cast<std::chrono::milliseconds>(end - start);
        std::cout << "MSA time: " << msa_duration.count() << "ms" << "\n";
        double ratio = (indel_duration.count()*1.0/(indel_duration+msa_duration).count() *1.0);
        std::cout << "indel percent: " << ratio *100.0 << "\n";
        std::cout << "MSA length is: " << msa.getMSAlength() << "\n";

        start = std::chrono::high_resolution_clock::now();
        std::string msaStr = msa.generateMsaString();
        end = std::chrono::high_resolution_clock::now();
        auto string_duration = std::chrono::duration_cast<std::chrono::milliseconds>(end - start);
