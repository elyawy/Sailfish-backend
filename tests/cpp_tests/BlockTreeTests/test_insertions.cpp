...
int main() {
    ...
    size_t event_position_1 = 0; // left edge of the sequence
    tree.handle_event(INSERTION, event_position_1, 1);

    size_t event_position_2 = 5; // middle of the sequence
    tree.handle_event(INSERTION, event_position_2, 1);

    size_t event_position_3 = 5; // middle of the sequence, right before the previous insertion
    tree.handle_event(INSERTION, event_position_3, 1);

    size_t event_position_4 = 8; // middle of the sequence, one site after the previous insertions
    tree.handle_event(INSERTION, event_position_4, 1);

    size_t event_position_5 = 13; // right edge of the sequence
    tree.handle_event(INSERTION, event_position_5, 1);
    ...
    assert(evolved_sequence == "ANRRRRNNRNRRRRN");
