...
int main() {
    ...
    std::string rootString(rootLength, 'M');
    ...
    mFac.setSiteRateModel({0.01, 0.0,0.0,0.0},
                          {0.25, 0.25, 0.25, 0.25},
                          {
    ...
    BlockMap blockmap = sim.generateSimulation();

    auto msa = MSA(blockmap, tree_.getRoot(), saveList);
    int msaLength = msa.getMSAlength();
    
    auto fullContainer = sim.simulateSubstitutions(msaLength, rootString, msa.getRootPositionsInMsa());
