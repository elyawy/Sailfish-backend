...
def build_auto_gamma_transition_matrix(
    ...
    for p in probs:
        if p == 0:
            ...
            normal_thresholds.append(norm.ppf(p))
...
def calculate_discrete_gamma_correlation(
    ...
    for i in range(K):
        ...
        if lower_p == 0:
            ...
            lower_q = gamma.ppf(lower_p, alpha, scale=1/alpha)

        upper_q = gamma.ppf(upper_p, alpha, scale=1/alpha)

        def integrand(x):
            return x * gamma.pdf(x, alpha, scale=1/alpha)
